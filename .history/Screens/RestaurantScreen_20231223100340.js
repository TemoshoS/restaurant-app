...
const RestaurantScreen = ({ restaurants, fetchRestaurants, navigation }) => {
  ...
  const handleAdd = async () => {
    try {
      ...
      if (!newRestaurantData.restName || !newRestaurantData.restLocation || !newRestaurantData.restInfo || !newRestaurantData.restPhone|| !newRestaurantData.restWebsite || !newRestaurantData.restLocation) {
          ...
          <View style={styles.restaurantItem}>
            ...
            {!isAdmin && (
              <TouchableOpacity style={styles.heartButton} onPress={() => handleLike(item.id)}>
                <Ionicons name="heart" size={28} color="#83764F" />
              </TouchableOpacity>
            )}
