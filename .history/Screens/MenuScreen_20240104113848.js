...
const MenuScreen = ({ route }) => {
  ...
  const filterMenuItems = (category) => {
    const filteredItems = category === 'all' ? menuItems : menuItems.filter((item) => item.menuCate === category);
  
    // If there are no items or only one item, return an empty array to prevent issues
    return filteredItems.length > 1 ? filteredItems : [];
  };
